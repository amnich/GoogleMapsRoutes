...
<#
.SYNOPSIS
    Google Maps Route & Map Generator — Advanced WPF Dark Mode application.
    Supports manual route calculation (Start, End, Waypoints, Fastest/Shortest/Eco)
    and batch processing of multi-format data files (JSON, CSV, Excel).

.DESCRIPTION
    Features:
      - Manual Mode (Manual Input):
          * Origin (start) and destination (end) points (with geocoding and validation)
          * Dynamic waypoint list (add, remove, reorder)
          * Route optimization selection: Fastest, Shortest, Eco-friendly (Eco)
          * Vehicle engine type selection for Eco routes (Gasoline, Diesel, Hybrid, Electric)
          * Instant route calculation with distance (km) and travel duration (min/hours)
          * Interactive static PNG map preview with highlighted route and numbered markers
          * Copy and open direct Google Maps navigation link in web browser
      - Batch Mode (Data Source / Batch Processing):
          * Multi-format data file support: Excel (.xlsx, .xls), CSV (.csv, .tsv), JSON (.json)
          * Automatic schema detection and customizable column mapping
          * Real-time input data preview table (DataGrid)
          * Progress bar, percentage, elapsed time, and real-time asynchronous event log
          * Detailed results table with status, points detail, and direct access to generated maps
          * Batch report export to Excel, CSV, and JSON
      - Security and Settings:
          * Secure API key storage using Windows DPAPI per-user encryption
          * Non-blocking asynchronous API key validation tester
          * Configurable default map dimensions, output directories, and route preferences
          * Fully customizable map overlay banners (Top/Bottom, alignment, line order)
          * Dynamic theme switching (Dark Mode / Light Mode)
          * Multi-language UI and API localization (English, German, Polish)
      - PS2EXE compatibility (compiles to standalone .EXE without external dependencies).
...
# ══════════════════════════════════════════════════════════════════════════════
# 4. STANDALONE BASE FUNCTIONS (EMBEDDED FOR PS2EXE COMPATIBILITY)
# ══════════════════════════════════════════════════════════════════════════════
...
function Import-RouteDataFile {
    ...
    # Check if file represents a sequential stops sequence (SequentialStops)
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. CONFIGURATION AND DPAPI SECURITY
# ══════════════════════════════════════════════════════════════════════════════
...
function Load-LocalizationConfig {
    ...
    if (-not [string]::IsNullOrWhiteSpace($script:EmbeddedLocalizationJson)) {
        try {
            ...
            Write-AppLog "Error parsing embedded localization catalog: $($_.Exception.Message)" "WARN"
        }
    }

    # 2. Check on-disk localization file and merge/override
...
function Write-AppLog {
    ...
    try {
        [System.IO.File]::AppendAllText($script:LogFile, "$entry`r`n", [System.Text.Encoding]::UTF8)
    }
    catch { }
...
Write-AppLog "================================================================================" "INFO"
Write-AppLog "Starting Google Maps Route & Map Generator" "INFO"
Write-AppLog "Environment: PowerShell $($PSVersionTable.PSVersion)" "INFO"
Write-AppLog "Configuration file: $script:ConfigFile" "INFO"
Write-AppLog "Event log file: $script:LogFile" "INFO"
...
# ══════════════════════════════════════════════════════════════════════════════
# 6. THREAD-SAFE BACKGROUND RUNSPACE WORKER FACTORY (INITIALSESSIONSTATE)
# ══════════════════════════════════════════════════════════════════════════════
...
function New-WorkerPowerShell {
    ...
    $ps.Runspace = $rs
    # IMPORTANT: [void] or $null = prevents PowerShell object leakage into function pipeline.
    # Without this, the function returns array @($ps, $ps), which on calling .BeginInvoke()
    # attempts to re-invoke the same instance and errors with:
...
# ══════════════════════════════════════════════════════════════════════════════
# 7. WPF XAML USER INTERFACE DEFINITION (MODERN DARK THEME)
# ══════════════════════════════════════════════════════════════════════════════
...
# ── 9. Retrieve UI Element References ────────────────────────────────────────
...
# ── 10. Multi-Language System and UI Helper Functions ────────────────────────
...
# Initialize control values from saved configuration
...
# Set map overlay control states
...
# ── 11. Event Handlers: Settings & API Key ───────────────────────────────────
...
$btnTestApiKey.Add_Click({
        ...
        # Asynchronous connection test to Google API in separate runspace
        $testScript = {
        ...
        $testTimer.Add_Tick({
                ...
                elseif ($script:TestTimerTicks -ge 200) {
                    # 20s timeout limit
                    if ($script:ActiveTestTimer) { try { $script:ActiveTestTimer.Stop() } catch { } }
                    $btnTestApiKey.IsEnabled = $true
...
# ── 12. Event Handlers: Tab 1 (Manual Input) ────────────────────────────────
...
# ── 13. Event Handlers: Tab 2 (Batch Processing) ─────────────────────────────
